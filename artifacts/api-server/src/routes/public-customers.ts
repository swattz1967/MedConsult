 ...
 */
router.post("/public/customers", async (req, res, next): Promise<void> => {
  ...
  try {
    ...
    (async () => {
      ...
      req.log.error({ err, customerId: customer.id }, "Failed to send welcome email/webhook after public customer registration");
    });
  } catch (err) {
    next(err);
  }
